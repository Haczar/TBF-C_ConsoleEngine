...
Data()
	...
	CTS_CWString Intro_Title         = L"Trial By Fire Engine";
	...
	Ptr( Cell) Intro_Title_RenderCells   = NULL;
...
fn returns(void) ChangeTitleTo_Grey()
{
	...
	{
		Intro_Title_RenderCells[cellIndex].Attributes = FOREGROUND_INTENSITY;
	}
}
...
fn returns(void) ChangeTitleTo_White()
{
	for (DataSize cellIndex = 0; cellIndex < Title_Length; cellIndex++)
	{
		Intro_Title_RenderCells[cellIndex].Attributes = Console_WhiteCell;
	}
}
...
fn returns(void) IntroState_Load parameters(void)
{
	...
	Timer_TillIntroFadeToGrey.EndTime = Timer_TillTitle.EndTime + 5.2L;
	...
	Title_Length         = wcslen(Intro_Title);
	EngineVersion_Length = wcslen(EngineVersion);

	Intro_Title_RenderCells   = GlobalAllocate(Cell, Title_Length        );
	Version_RenderCells = GlobalAllocate(Cell, EngineVersion_Length);

	for (DataSize cellIndex = 0; cellIndex < Title_Length; cellIndex++)
	{
		Intro_Title_RenderCells[cellIndex].Char.UnicodeChar = Intro_Title[cellIndex];
	}
...
fn returns(void) IntroState_Render parameters(void)
{
	...
	if (RenderTitle)
	{
		...
		Renderer_WriteToBufferCells(Intro_Title_RenderCells, startingCell, endingCell);
	}
...
fn returns( Ptr(StateObj)) GetState_Intro parameters(void)
{
	Stack()
