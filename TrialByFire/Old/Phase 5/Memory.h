...
fn returns(void) MemoryBlockArray_Add         parameters(Ptr(MemoryBlockArray) _memoryArray, Ptr(MemoryBlock) _heap);
...
fn returns(Address) Internal_ScopedAllocate parameters(Ptr(MemoryBlock) _scopedMemory, DataSize _sizeOfAllocation);
fn returns(void   ) ScopedDeallocate        parameters(Ptr(MemoryBlock) _scopedMemory);
